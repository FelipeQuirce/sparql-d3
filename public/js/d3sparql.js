//
// js/d3sparql.js - utilities for visualizing SPARQL results with the D3 library
//
...
var d3sparql = {
  version: "js/d3sparql.js version 2018-05-04",
...
d3sparql.fetch = function(url, callback) {
  ...
  d3.request(url)
  .mimeType(mime)
  .header("Authorization", "Basic "+btoa("admin" + ":" + "xso2tk6r@DJ1"))
...
d3sparql.graph = function(json, config) {
  ...
  for (var i = 0; i < data.length; i++) {
    ...
    if (!check.has(key1)) {
      graph.nodes.push({"key": key1, "label": label1, "value": value1, id: index})
      check.set(key1, index)
      index++
    }
    if (!check.has(key2)) {
      graph.nodes.push({"key": key2, "label": label2, "value": value2, id:index})
...
d3sparql.forcegraph = function(json, config) {
  ...
  var svg = d3sparql.select(opts.selector, "forcegraph").append("div")
